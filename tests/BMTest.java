...
class BMTest extends TestDriver {
  ...
  protected boolean test1() {
    ...
    try {
      ...
      System.err.print("*** TEST1: Could not allocate " + numPages);
    ...
    try {
      ...
      System.err.print("*** TEST1: Could not unpin the first new page.\n");
      e.printStackTrace();
      status = FAIL;
    }
    ...
    for (int i = firstPid.pid; status == PASS && i < lastPid.pid; i++) {
      ...
      try {
        ...
        status = FAIL;
        System.err.print("*** TEST1: Could not pin new page " + pid.pid + "\n");
        e.printStackTrace();
      }
      ...
      if (status == PASS) {
        ...
        if (status == PASS) {
          try {
            ...
            status = FAIL;
            System.err
                .print("*** TEST1: Could not unpin dirty page " + pid.pid + "\n");
            e.printStackTrace();
          }
        }
      }
    ...
    for (int i = firstPid.pid; status == PASS && i < lastPid.pid; i++) {
      ...
      try {
        ...
        status = FAIL;
        System.err.print("*** TEST1: Could not pin page " + pid.pid + "\n");
        e.printStackTrace();
      }
      ...
      if (status == PASS) {
        ...
        if (status == PASS) {
          if (data != (pid.pid) + 99999) {
            status = FAIL;
            System.err.print("*** TEST1: Read wrong data back from page " + pid.pid
                + "\n");
          }
        }
        ...
        if (status == PASS) {
          try {
            ...
            status = FAIL;
            System.err.print("*** TEST1: Could not unpin page " + pid.pid + "\n");
            e.printStackTrace();
          }
        }
      }
    ...
    if (status == PASS) {
      ...
      for (int i  = firstPid.pid; i < lastPid.pid; i++) {
        ...
        try {
          ...
          status = FAIL;
          System.err.print("*** TEST1: Error freeing page " + pid.pid + "\n");
          e.printStackTrace();
        }
      }
    }
  ...
  protected boolean test2() {
    ...
    try {
      ...
      System.err.print("*** TEST2: Could not allocate " + numPages);
      System.err.print(" new pages in the database.\n");
      e.printStackTrace();
      return false;
    }
    ...
    for (int i = firstPid.pid + 1; status == PASS && i < lastPid.pid; i++) {
      ...
      try {
        ...
        status = FAIL;
        System.err.print("*** TEST2: Could not pin new page " + pid.pid + "\n");
        e.printStackTrace();
      }
    }
    ...
    if (status == PASS && Minibase.BufferManager.getNumUnpinned() != 0) {
      status = FAIL;
      System.err.print("*** TEST2: The buffer manager thinks it has "
    ...
    if (status == PASS) {
      ...
      if (status == PASS) {
        status = FAIL;
        System.err.print("TEST2: The expected exception was not thrown\n");
      } else {
        status = PASS;
      }
    }
    ...
    if (status == PASS) {
      try {
        ...
        status = FAIL;
        System.err.print("*** TEST2: Could not acquire a second pin on a page\n");
        e.printStackTrace();
      }
    ...
    for (int i = firstPid.pid; i <= lastPid.pid; i++) {
        ...
        status = FAIL;
        System.err.print("*** TEST2: Error freeing page " + pid.pid + "\n");
        e.printStackTrace();
      }
    }
  ...
  protected boolean test3() {
    ...
    for (index = 0; status == PASS && index < numPages; ++index) {
      try {
        ...
        status = FAIL;
        System.err.print("*** TEST3: Could not allocate new page number " + index + 1
            + "\n");
        e.printStackTrace();
      }
        ...
        if (status == PASS) {
          if (pid.pid % 20 != 12) {
            try {
              ...
              status = FAIL;
              System.err.print("*** TEST3: Could not unpin dirty page " + pid.pid
                  + "\n");
            }
          }
        }
    ...
    if (status == PASS) {
      ...
      for (index = 0; status == PASS && index < numPages; ++index) {
        ...
        try {
          ...
          status = FAIL;
          System.err.print("*** TEST3: Could not pin page " + pid.pid + "\n");
          e.printStackTrace();
        }
        ...
        if (status == PASS) {
          ...
          if (data != pid.pid + 99999) {
            status = FAIL;
            System.err.print("*** TEST3: Read wrong data back from page " + pid.pid
                + "\n");
          }
        }
        ...
        if (status == PASS) {
          try {
            ...
            status = FAIL;
            System.err.print("*** TEST3: Could not unpin page " + pid.pid + "\n");
            e.printStackTrace();
          }
        }
